...
public class GroovyScriptFactoryTests {
	...
	public void testStaticScript() throws Exception {
		...
		assertThat(ctx.getBeansOfType(Calculator.class).containsValue(calc)).isTrue();
		assertThat(ctx.getBeansOfType(Messenger.class).containsValue(messenger)).isTrue();
	}
	...
	public void testRefreshableFromTagProxyTargetClass() throws Exception {
		...
		assertThat(ctx.getBeansOfType(ConcreteMessenger.class).containsValue(messenger)).isTrue();
