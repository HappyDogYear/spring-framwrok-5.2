...
public class Jackson2ObjectMapperBuilderTests {
	...
	public void wellKnownModules() throws JsonProcessingException, UnsupportedEncodingException {
		...
		assertThat(new String(objectMapper.writeValueAsBytes(dateTime), StandardCharsets.UTF_8)).isEqualTo(timestamp.toString());

		Path file = Paths.get("foo");
		assertThat(new String(objectMapper.writeValueAsBytes(file), StandardCharsets.UTF_8).endsWith("foo\"")).isTrue();
	...
	public void customizeWellKnownModulesWithModule()
		...
		assertThat(new String(objectMapper.writeValueAsBytes(dateTime), StandardCharsets.UTF_8)).isEqualTo("1322903730000");
		assertThat(new String(objectMapper.writeValueAsBytes(4), StandardCharsets.UTF_8)).contains("customid");
	}
